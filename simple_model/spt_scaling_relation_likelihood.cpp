...
// How to successfully import the data from the existing dataset online
// to theta?

// Also how would the value of theta change

// For example, how to do we intend to import the WMAP9 (which contains the values of
// omega_m, omega_l, and omega_k ? HMF from collosus?

// Or even importing astropy and collosus in general to C++?
...
float M0 = 3e14;
float Ez0 = 1.0;
const float _omega_m = 0.3153;
const float _omega_l = 0.6847;
const float _omega_k = -0.0096;
...
// Is z being properly implemented in the EZ_sq function?
float ln_zeta_given_M(float M, float z){
    float currentResult = y3_cluster::EZ_sq(_omega_m,_omega_l,_omega_k);
    return (log(theta[0]) + (theta[1]) * log(M/M0) + (theta[2]) * (log(currentResult)/Ez0));
}

// Is z being properly implemented in the EZ_sq function?
float ln_lbd_given_M(float M, float z) {
    double currentResult = y3_cluster::EZ_sq(_omega_m,_omega_l,_omega_k);
    return (log(theta[4]) + (theta[5]) * log(M/M0) + (theta[6]) * (log(currentResult)/Ez0));
}
